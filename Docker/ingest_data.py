...
try:
    engine = create_engine(
        "postgresql://root:root@localhost:5435/ny_taxi"
...
# loading the data
try:
    data.to_sql(name="green_taxi_data", con=engine, if_exists="replace")
except Exception as e:
...
try:
    dataset_with_zones.head(n=0).to_sql(con=engine, name="zones", if_exists="replace")
except Exception as e:
    print(f"Failed to load schema into the database:, {e}")
else:
    print("schema loaded successfully into the database")
...
try:
    dataset_with_zones.to_sql(con=engine, name="zones", if_exists="replace")
except Exception as e:
    print(f"Failed to load data into the database:, {e}")
else:
    print("data loaded successfully into the database")
